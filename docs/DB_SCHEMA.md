...
Added 2026-08-27 (`db/migrations/2026-08-27_bulk_apply_transactional.sql`). It is
what makes `/bulk_insert` and `/bulk_update` transactional: the whole payload is
one statement, so a rejected record rolls every record back. Returns
`[{table, index, name, before, after}]` — the bot builds its field-diff report
from `before`/`after`.

Three things about it that are load-bearing:

- **`security invoker`, deliberately.** It takes a table name and builds dynamic
  SQL from it. As `security definer` it would be a write-anything endpoint for
  every role that can reach PostgREST. EXECUTE is also revoked from `anon` and
...
- **Only the columns present in each record are written.** The obvious
  ...
  instead of taking its default — `id` and `created_at` included.

Verified by `db/tests/bulk_apply_test.sql` against a throwaway Postgres, not by
the bot's pytest suite. Changing the function without running that script is
untested.
